...
def main():
    ...
    parser.add_argument("--iterations", type=int, default=10000, help="iterations number, default: 10000")
    parser.add_argument("--epsilon", type=float, default=.2, help="epsilon number, default: .2")
    parser.add_argument("--alpha", type=float, default=.7, help="alpha number, default: .7")
    parser.add_argument("--discount", type=float, default=.9, help="discount number, default: .9")
    parser.add_argument("--ai", type=str, choices=['t', 'p'], help="use AI agent to train or play game")
...
def mainGame(movementInfo):
    ...
    while True:
        ...
        if len(upperPipes) > 0 and upperPipes[0]['x'] < -IMAGES_INFO['pipe'][WIDTH]:
            upperPipes.pop(0)
            lowerPipes.pop(0)
        if UI == True:
            # draw sprites
...
def showGameOverScreen(crashInfo):
    ...
    playery = crashInfo['y']
    playerHeight = IMAGES_INFO['player'][0][HEIGHT]
...
# def get_successors(playerx, playery, basex, playerIndexGen):
...
#         # remove first pipe if its out of the screen
#         if len(upperPipes) > 0 and upperPipes[0]['x'] < -IMAGES_INFO['pipe'][0][WIDTH]:
#             upperPipes.pop(0)
#             lowerPipes.pop(0)
...
#         playerHeight = IMAGES_INFO['player'][playerIndex][HEIGHT]
#         playery += min(playerVelY, BASEY - playery - playerHeight)
#         successors.append((playerx, playery, playerVelY, lowerPipes, event))
